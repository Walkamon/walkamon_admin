...
function ShopItemEditForm({ itemOptions, initialData, onSubmit, onClose }) {
  ...
  const activeItems = itemOptions.filter((item) => item.isActive);
  ...
  return (
    <form onSubmit={handleSubmit} className="item-form-layout">
      <div className="rounded-xl border border-border bg-muted/40 p-4">
        ...
        <p className="mt-1 font-semibold text-foreground">{initialData?.itemName || "-"}</p>
...
export function ShopPage() {
  ...
  const handleUpdateShopItem = async (formData) => {
    ...
    try {
      const response = await shopApi.update(editingItem.shopItemId, {
        itemId: formData.itemId,
        priceAmount: formData.priceAmount,
      });
      ...
      setShopItems((prev) =>
        prev.map((item) =>
          item.shopItemId === editingItem.shopItemId ? { ...item, ...updatedItem } : item,
        ),
      );
      setEditingItem(null);
  ...
  const handleDeactivate = async () => {
    ...
    try {
      ...
      setShopItems((prev) =>
        prev.map((item) =>
          item.shopItemId === deleteTarget.shopItemId
            ? { ...item, isActive: false }
            : item,
        ),
      );
  ...
  const handleActivate = async (item) => {
    ...
    try {
      const response = await shopApi.activate(item.shopItemId, {
        itemId,
      ...
      setShopItems((prev) =>
        prev.map((shopItem) =>
          shopItem.shopItemId === item.shopItemId
            ? { ...shopItem, ...updatedItem, isActive: true }
            : shopItem,
        ),
      );
      ...
      <div className="header-wrapper">
        <div>
          ...
          <p className="header-subtitle">
            Quản lý vật phẩm bán trong shop Walkamon
          </p>
        </div>
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              pagedItems.map((item) => {
                ...
                const itemName =
                  pickField(item, "itemName", "ItemName") ?? itemInfo?.itemName ?? "-";
                const itemTypeName = getItemTypeName(item, itemInfo);
          ...
          <div className="item-form-layout">
            <div className="rounded-xl border border-destructive/20 bg-destructive/10 p-4">
              <p className="text-sm text-destructive">
                Shop item này sẽ bị vô hiệu hóa và ẩn khỏi danh sách bán.
              </p>
              <p className="mt-2 font-semibold text-foreground">
                {deleteTarget.itemName || "-"}
              </p>
              <p className="mt-1 text-xs text-muted-foreground font-mono">
                #{deleteTarget.shopItemId}
              </p>
            </div>
